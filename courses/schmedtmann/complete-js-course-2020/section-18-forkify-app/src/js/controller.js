...
// parentEl.innerHTML = '';
// then we render the spinner and then the parent element, in this case, the recipe container:
...
// one more scenario:
// if we don't have any hash, we then get an error
// our spinner keeps showing that the application is loading 
// we are trying to read the ID but the location doesn't have a hash
// we have an empty string for which we are trying to find a corresponding recipe
// solution is to put a simple guard clause right after showRecipe try:
// if (!id) return;
// this is the modern way of performing this test
// the previous way would have to write if id and then wrap of the following code the if block, creating unnecessary nesting making our code harder to read
...
// we'll hit Command + D and replace all recipe. with this.#data.
...
// if(!query) return;
// await will be changed from looking for 'pizza' to query
/*
const controlSearchResults = async function() {
  try {
...
// then, here in controller, we can immediately import that without having to create the instance manually:
// import resultsView from './views/resultsView.js';
// now we should be able to display the spinner
// let'd do that at the very beginning:
/*
const controlSearchResults = async function()  try {
...
*/
// now we need to fill it with real data: id, image, publisher, and title
// to start, we need the id here in order to build the link so when we click it, that will change the hash in the URL, which, in turn, will trigger the loading of a recipe
